...
const handleSubmit = async () => {
    try {
        ...
        if (error.response && error.response.data && error.response.data.message) {
            toast.error(error.response.data.message); // Affiche le message d'erreur de l'API
        } else {
            toast.error('Une erreur est survenue lors de l\'inscription.'); // Message d'erreur générique
        }
    }
};
</script>
...
<style scoped>
/* Ajoutez votre CSS ici pour personnaliser le formulaire si nécessaire */
</style>